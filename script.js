...
// Set up intersection observer for lazy loading iframes
function setupIntersectionObserver() {
    intersectionObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                const iframe = entry.target;
                const playlistId = iframe.dataset.playlistId;
                
                if (playlistId && !loadedIframes.has(playlistId)) {
                    iframe.src = `https://www.youtube.com/embed/videoseries?list=${playlistId}&rel=0`;
                    loadedIframes.add(playlistId);
                }
                
...
    setupCardEventListeners(card, playlistId);
    
    return card;
}


// Set up event listeners for a playlist card
function setupCardEventListeners(card, playlistId) {
...
        
        // Load iframe when card is expanded for the first time
        if (!wasExpanded && iframe && playlistId && !loadedIframes.has(playlistId)) {
            iframe.src = `https://www.youtube.com/embed/videoseries?list=${playlistId}&rel=0`;
            loadedIframes.add(playlistId);
        }
    });
...
    });
    
    // Observe iframe for lazy loading when it comes into view
    if (iframe && playlistId) {
...
    });
    
    // Clear loaded iframes when filtering
    loadedIframes.clear();
    
